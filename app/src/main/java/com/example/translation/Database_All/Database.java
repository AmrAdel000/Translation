...
public class Database extends SQLiteOpenHelper {
    ...
    public static final String DP_name = "Translate";
    public static final int DP_Vr = 3;
    public static final String NOTE_TB_NAME = "Words";
    ...
    public Model getWords_2(String text, int x) {
        try {
            String args[] = {"amr"};
            //   "SELECT * FROM Words WHERE Arabic_word like 'amr' ;"
            SQLiteDatabase dp = getReadableDatabase();
            Cursor c = dp.rawQuery("SELECT * FROM " + NOTE_TB_NAME, args);
            Model model;
            c.move(x);
            int id = c.getInt(0);
            String note = c.getString(1);
            ...
            Toast.makeText(ct, note, Toast.LENGTH_SHORT).show();
            return model;
    ...
    public Model search_for_en(String search) {
        SQLiteDatabase dp = getReadableDatabase();
